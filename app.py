...
def image_to_spectrogram(img1, img2):
    ...
    # ✅ UPDATED (better spectrogram clarity)
...
# ---------------- LIVE CAMERA ----------------
    ...
    if img1 and img2:
        ...
        This spectrogram is generated by analyzing motion differences between two consecutive frames.
        The pixel variations are converted into a signal and processed using STFT.

        - X-axis → Time
        - Y-axis → Frequency
        - Color → Motion intensity

        Walking → Repeated patterns  
        Standing → Flat signal  
        ...
        # ✅ Medium prediction display
        st.markdown(f"## 🧠 Prediction: {predicted_class.upper()}")
        st.markdown(f"### 🎯 Confidence: {confidence:.2f}%")

        # ✅ Trend feature
        st.markdown("### 📈 Live Activity Trend")
        if len(st.session_state.history) > 1:
            trend_df = pd.DataFrame(st.session_state.history)
            fig_trend = px.line(trend_df, y="Confidence", title="Confidence Trend")
